  ...
  const PackageBadge = ({
    ...
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-2xl border font-black transition hover:-translate-y-0.5 ${
        ...
        isGold
          ? 'border-[#FFD84D]/70 bg-gradient-to-b from-[#6B500E] to-[#352000] text-[#FFE680] shadow-[0_4px_0_#7A5600]'
  ...
  return (
    <>
      ...
      <header className="fixed inset-x-0 top-0 z-50 hidden bg-transparent px-5 pt-4 text-white lg:block">
        <div
          dir="ltr"
          className="relative mx-auto grid min-h-[78px] max-w-[1280px] grid-cols-[1fr_auto_1fr] items-center gap-4 overflow-visible rounded-[28px] border border-[#7868D9]/45 bg-[linear-gradient(135deg,rgba(18,8,42,0.97),rgba(31,12,67,0.94))] px-5 py-3 shadow-[0_10px_0_rgba(44,32,111,0.75),0_28px_70px_rgba(5,0,20,0.52)]"
        >
          ...
          <nav
            dir="rtl"
            className="flex items-center gap-2 rounded-2xl border border-white/[0.07] bg-black/10 p-1.5"
          >
            {navigationItems.map((item) => (
              <button
                ...
                className="rounded-xl px-5 py-2.5 text-sm font-black text-white/80 transition hover:bg-white/10 hover:text-[#FACC15]"
              >
                {item.label}
              </button>
            ))}
          ...
          >
            <button
              type="button"
              onClick={openAccount}
              className="inline-flex h-11 max-w-[190px] items-center gap-2.5 rounded-2xl border border-[#FACC15]/40 bg-gradient-to-b from-[#F9D441] to-[#E8AC00] px-5 font-black text-[#28104B] shadow-[0_5px_0_#8A6200]"
            >
              {user ? (
            ...
            <button
              type="button"
              onClick={openAccount}
              className="flex h-11 w-11 items-center justify-center rounded-2xl border border-[#7D70D9]/45 bg-gradient-to-b from-[#302A68] to-[#211A4D] text-[#FACC15] shadow-[0_4px_0_#151132]"
            >
      ...
      <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 text-white lg:hidden">
        <div className="mx-auto flex min-h-[62px] items-center justify-between gap-1.5 rounded-[21px] border border-[#7868D9]/40 bg-[linear-gradient(135deg,rgba(16,7,37,0.97),rgba(28,11,61,0.96))] px-2.5 py-2.5 shadow-[0_6px_0_rgba(43,31,105,0.72)]">
          <button
            type="button"
            onClick={openAccount}
            className="inline-flex h-10 max-w-[105px] items-center justify-center gap-1.5 rounded-xl border border-[#FACC15]/35 bg-gradient-to-b from-[#F9D441] to-[#E8AC00] px-2.5 font-black text-[#28104B] shadow-[0_3px_0_#8A6200]"
          >
            {user ? (
              ...
              <nav className="grid gap-3">
                {navigationItems.map((item) => (
                  <button
                    ...
                    className="rounded-2xl border border-white/10 bg-white/[0.055] px-5 py-4 text-right font-black text-white/90"
                  >
                    {item.label}
                  </button>
                ))}
