...
export default function Hero({ onNavigate }: HeroProps) {
  return (
    <section
      dir="rtl"
      className="relative min-h-[760px] overflow-hidden bg-[linear-gradient(180deg,#35116D_0%,#291052_52%,#210A43_100%)] pt-24 text-white sm:min-h-screen lg:pt-24"
    >
      {/* خلفية خفيفة بدون الدوائر الكبيرة */}
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {[...Array(8)].map((_, index) => (
          <span
            key={index}
            className="absolute h-2 w-2 rounded-full bg-[#FACC15]/25"
            style={{
              right: `${8 + ((index * 17) % 84)}%`,
              top: `${12 + ((index * 19) % 72)}%`,
            }}
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-5rem)] max-w-[1280px] items-center gap-7 px-5 pb-16 pt-20 sm:px-7 lg:grid-cols-2 lg:gap-12 lg:px-12 lg:pb-12 lg:pt-12">
        ...
        >
          ...
          <div className="mx-auto grid max-w-[620px] grid-cols-1 gap-3 sm:grid-cols-2 lg:mx-0">
            <button
              ...
              className="inline-flex min-h-14 items-center justify-center gap-3 rounded-2xl bg-[#FACC15] px-6 text-lg font-black text-[#2E1065] shadow-[0_6px_0_#906900] transition hover:-translate-y-0.5"
            >
            ...
            <button
              ...
              className="inline-flex min-h-14 items-center justify-center gap-3 rounded-2xl border border-white/20 bg-white/[0.07] px-6 text-lg font-bold text-white transition hover:bg-white/10"
            >
          ...
          <div className="mt-7 hidden flex-wrap justify-center gap-2.5 text-sm sm:flex lg:justify-start">
            {['فريقان أو أكثر', 'فئات متنوعة', 'أسئلة وتحديات', 'نقاط ونتائج'].map(
              (item) => (
                <span
                  key={item}
        ...
        >
          <div className="relative flex w-full max-w-[470px] items-center justify-center sm:max-w-[520px]">
            <img
              src="/almaydan-logo.png?v=20"
